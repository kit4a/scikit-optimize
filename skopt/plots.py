...
def plot_gaussian_process_2D(res, **kwargs):
    """Plots the optimization results and the gaussian process
    for 2-D objective functions as heatmaps.
    ...
    assert n_dims == 2, "Space dimension must be 2"
    dimension_x = res.space.dimensions[0]
    dimension_y = res.space.dimensions[1]
    x, x_model = _evenly_sample(dimension_x, n_points)
    y, y_model = _evenly_sample(dimension_y, n_points)
    x_plot, y_plot = np.meshgrid(x, y)
    X = np.array([np.array([xe,ye]).reshape(1,-1) for ye in y for xe in x]).reshape(-1, 2)
    X_model = np.array([np.array([xe,ye]).reshape(1,-1) for ye in y_model for xe in x_model]).reshape(-1, 2)
    ...
    if n_calls < 0:
        ...
        curr_x_iters = np.array([e[0] for e in curr_X_iters])
        curr_y_iters = np.array([e[1] for e in curr_X_iters])
    ...
    if show_acq_func:
        ...
        if show_next_point and next_X is not None:
            ax.plot(next_X[0], next_X[1], marker="o", markeredgecolor="red",
...
def plot_gaussian_process_3Dsurface(res, **kwargs):
    ...
    n_points : int, default: 1000
        Number of data points per direction used to create the plots
    ...
    if show_mu:
        z_pred = model.predict(X_model)
        z_plot = z_pred.reshape(x_plot.shape)
        surf = ax.plot_surface(x_plot, y_plot, z_plot, cmap=cm.viridis, alpha=0.65)


    # Plot sampled points
    if show_observations:
        ax.scatter(curr_x_iters, curr_y_iters, curr_func_vals,
                marker=".", c="r", s=35, label="Observations", zorder=-1)
